...
ТАБЛИЦА 1. Диапазоны изменения концентраций компонентов растворов, моделирующих вина.
...
![image](https://github.com/user-attachments/assets/cf26e681-5739-413e-893f-20cabd905ffe)
...
Рис.1. ИК-Фурье спектрометр Bruker.

![Рисунок1](https://github.com/user-attachments/assets/53cbbf7d-1520-49de-a4d3-594eeaeccd5c)

Рис.2. Схема проведения измерений спектров ИК-поглощения в системе прокачки.
...
Примеры спектров ИК-поглощения можно найти в файле 'wines_spec_demo.csv', а соответствующие им концентрации - в файле 'wines_conc_demo.csv'.
...
Количество целевых переменных: 5. Диапазоны изменения концентраций 5 компонентов соответствуют величинам, указанным в Таблице 1. Распределение примеров по концентрациям представлено ниже:

![распределение примеров](https://github.com/user-attachments/assets/460c3434-dc06-4b43-8fff-f1c58a3a11bd)

Рис.3. Распределения примеров по концентрациям целевых переменных в полученном наборе данных.

Резюме: для всех компонентов модельных растворов в полученном на данный момент наборе присутствуют все 5 точек по концентрациям, однако очень большие опасения вызывает распределение примеров по концентрациям для этанола. Можно предполагать, что модель будет хорошо предсказывать низкие концентрации этанола и плохо -  большие.

### Удаление выбросов

Удаление выбросов происходило на основе анализа проекции набора данных в пространство первых трех главных компонент (анализ главных компонент) и поканальных статистических показателей массива данных (минимальное, максимально, медианное и среднее значение признака в спектральном канале).


До удаления выбросов:
![pca_raw](https://github.com/user-attachments/assets/13094570-4618-44f7-8d57-ebd602c930f3)

Рис.4. Проекция набора данных в пространство первых трех главных компонент до удаления выбросов.
...
Рис.5. Проекция набора данных в пространство первых трех главных компонент после удаления выбросов.

![image](https://github.com/user-attachments/assets/9ce52229-1c2a-473c-9d31-5b50dd32774f)

Рис.6. Поканальные статистические показатели набора данных. Красная закрашенная область соответствует стандартному отклонению значений оптической плотности, отложенному от среднего значения оптической плотности в данном канале в обе стороны.
...
## 4. Методы машинного обучения для предсказания концентрации целевых параметров
...
## 5. Предсказание концентрации компонентов модельных растворов

На первом этапе для определения концентраций 5 компонентов вин в модельных растворах были использованы 3 алгоритма анализа данных: метод ПЛС-регрессии, МСП и СНС (Подробнее см. п.2). Для этого они были применены к набору спектров ИК-поглощения модельных растворов, полученных в схеме измерений в системе прокачки (1734 примера). Результаты применения натренированных на тренировочных поднаборах (1203 примера) моделей к примерам из тестовой выборки (171 пример) представлены на Рис..

![image](https://github.com/user-attachments/assets/4d86b4d3-2117-4800-a583-c26703aff351)

Рис.7. Результаты применения ПЛС-регрессии, МСП и СНС к набору спектров модельных растворов, измеренных на ИК-спектрометре в системе прокачки. а) Значение САО определения концентрации компонента на тестовом наборе данных, отнесенная к диапазону изменения концентрации компонента в наборе данных; б) Увеличение ошибки определения концентрации компонентов на тестовом наборе по сравнению с тренировочным.
...
Рис.8. Box-plot абсолютной ошибки предсказания концентраций компонентов реальных вин, предсказанных с помощью СНС.
